...
vectors = mlmc.graph.get_nmf(d["adjacency"]+1e-15,35)
vectors = mlmc.graph.get_random_projection(d["adjacency"],35)
#vectors = mlmc.graph.get_node2vec(d["adjacency"],35)

cooc = mlmc.graph.cooc_matrix(d["train"][1], classes)
mlmc.graph.helpers.correlate_similarity(cooc, vectors, 10, classwise=False)
