...
class BILSTMCNN(Sequence2SequenceAbstract):
    def __init__(self, vocabulary, weights,  classes, word_length=45, alphabet=string.ascii_letters+"0123456789",
                 dropout: float = 0.0,
        ...
        self.lstm = mlmc.layers.LSTM(weights.shape[1]+len(self.convs)*self.filters, self.lstm_hidden, dropout=0.5,
...
blc = BILSTMCNN(vocabulary, weights, data["classes"],optimizer=torch.optim.SGD,
                            ...
                            locked_dropout=0.0 ,word_dropout=0.01, dropout=0.65
                            )
