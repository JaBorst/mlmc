...
def test_threshold_scaling_mcut():
    ...
    ind1 = torch.tensor([0, 0, 0, 0, 1, 1])
    ind2 = torch.tensor([0, 1, 0, 0, 0, 1])
    assert torch.equal(thresholds.threshold_scaling_mcut(example1, ind1), torch.tensor([[1., 1., 1., 0., 1., 0.]]))
    assert torch.equal(thresholds.threshold_scaling_mcut(example2, ind2), torch.tensor([[1., 0., 1., 1., 1., 1.]]))
...
def test_threshold_wrapper():
    ...
    assert torch.equal(mlmc.thresholds.get(name="scaling_mcut", ind=ind).__call__(example5), torch.tensor([[1., 1., 1., 0., 0., 1.]]))
