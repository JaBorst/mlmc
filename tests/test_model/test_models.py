...
def test_KimCNN():
    assertion_function(model_type=mlmc.models.KimCNN, mode="untrainable", target="multi", filters=10, kernel_sizes=[3,4])

def test_XMLCNN():
    assertion_function(model_type=mlmc.models.XMLCNN, target="multi", mode="untrainable", filters=10, kernel_sizes=[3,4])

def test_LSAN_transformer():
    assertion_function(model_type=mlmc.models.LSANNC, target="multi", label_model="test",n_layers=1, hidden_representations=10, d_a=10)
