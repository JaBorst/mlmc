...
class LabelSelfAttention(torch.nn.Module):
    def __init__(self, hidden_features):
        super(LabelSelfAttention,self).__init__()
        self.hidden_features = hidden_features
...
class DynamicWeightedFusion(torch.nn.Module):
    def __init__(self, in_features, n_inputs, noise=0.01, dropout=0.2, norm="softmax", share_weights=False):
        ...
        if self.share_weights:
            ...
            self.weights_projection = torch.nn.ModuleList([torch.nn.Linear(in_features,1) for _ in range(self.n_inputs)])
...
class KnowledgeAwareSelfAttention(torch.nn.Module):
    def __init__(self, in_features, noise=0.01, dropout=0.2, norm="softmax", n_layers=3):
        super(KnowledgeAwareSelfAttention, self).__init__()
        self.in_features = in_features
        self.noise = noise
        self.dropout = dropout
        self.norm = norm
        ...
        assert norm in ("softmax","sigmoid","norm")
        self.gcn = torch.nn.ModuleList(
            [GraphConv(in_feats=self.in_features, out_feats=self.in_features)
             for _ in range(self.n_layers)]
        )
        self.dropout = dropout
        self.dropout_layer = torch.nn.Dropout(dropout)

    def forward(self, x, nodes, graph):
        att = torch.matmul(x,nodes.t())
        if self.norm == "softmax":
