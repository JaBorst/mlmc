...
class LSAN3BAG(TextClassificationAbstractGraph):
    """
    https://raw.githubusercontent.com/EMNLP2019LSAN/LSAN/master/attention/model.py
    """
    def __init__(self, classes, method="glove", scale="mean",hidden_representations=400,share_weighting=False, weight_norm ="norm", branch_noise = 0.001, representation="roberta", dropout=0.3, propagation_layers=3, d_a=200, max_len=400, n_layers=1, **kwargs):
        super(LSAN3BAG, self).__init__(**kwargs)
        #My Stuff
        ...
        self.dynamic_fusion = DynamicWeightedFusion(in_features=self.hidden_representations*2, n_inputs=3,share_weights=self.share_weighting, norm=self.weight_norm).to(self.device)
        ...
        self.gsa_p = torch.nn.Linear(self.hidden_representations * 2, 2* self.hidden_representations)

        self.output_layer = torch.nn.Linear(self.hidden_representations*2 , 100)
        self.output_layer2 = torch.nn.Linear(100 , 1)
    ...
    def forward(self, x, return_weights=False):
        ...
        graph_att = self.gsa_p(self.gsa(label_att, graph_embed, self.dgl_graph))

        doc, weights = self.dynamic_fusion([self_att[:,:self.n_classes],  label_att[:,:self.n_classes], graph_att[:,:self.n_classes]])

        doc = self.dropout_layer(doc)
        pred = self.output_layer2(torch.relu(self.output_layer(doc / self.label_embedding_dim))).squeeze()
        if self.log_bw:
