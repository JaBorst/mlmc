...
from mlmc.experimental.layers.Distances import Distance


class DLC(TextClassificationAbstract,TextClassificationAbstractZeroShot):
    """
    https://raw.githubusercontent.com/EMNLP2019LSAN/LSAN/master/attention/model.py
    """
    def __init__(self, classes, method="glove", scale="mean", norm=False, representation="roberta", use_lstm=False, d_a=200, max_len=400, n_layers=4, **kwargs):
        super(DLC, self).__init__(**kwargs)
        #My Stuff
        ...
        self.distances=["scalar","lrd"]
        self.distance_layer = Distance(distances=self.distances)
    ...
    def forward(self, x):
        ...
        outputs = embeddings#, hidden_state)
        labels = self.input_projection(self.label_embedding- self.label_embedding.mean(0)[None])

        self_att = self.self_attention(outputs, labels)
        prior = self.label_attention(outputs, labels)
        doc = self.dynamic_fusion(self_att, prior)
        doc = self.embedding_dropout(doc)
        pred = self.output_layer2(torch.relu(self.output_layer(doc).squeeze()))
    ...
    def label_attention(self,x, labels):
        m1 = torch.softmax(self.distance_layer(x, labels),-2)
        label_att = torch.stack([torch.bmm(m1[:,:,:,i].transpose(1,2), x) for i in range(len(self.distances))]).sum(0)
        return label_att
