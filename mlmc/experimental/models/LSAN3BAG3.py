...
class LSAN3BAG3(TextClassificationAbstractGraph):
    """
    https://raw.githubusercontent.com/EMNLP2019LSAN/LSAN/master/attention/model.py
    """
    def __init__(self, classes, method="glove", scale="mean",hidden_representations=400,share_weighting=False, weight_norm ="norm", branch_noise = 0.01, representation="roberta", dropout=0.3, propagation_layers=3, d_a=200, max_len=400, n_layers=1, **kwargs):
        super(LSAN3BAG3, self).__init__(**kwargs)
        #My Stuff
        ...
        self.projection_input = torch.nn.Linear(self.embeddings_dim, self.hidden_representations ).to(self.device)
        self.projection_labels = torch.nn.Linear(self.label_embedding_dim, self.hidden_representations ).to(self.device)

        self.lsatt = LabelSpecificSelfAttention(in_features=self.hidden_representations,in_features2=self.hidden_representations, hidden_features=self.d_a)
        self.lsatt2 = LabelSpecificSelfAttention(in_features=self.hidden_representations, in_features2=self.hidden_representations, hidden_features=self.d_a).to(self.device)


        self.latt = LabelSelfAttention(hidden_features=self.hidden_representations)
        self.gsa = GraphSpecificSelfAttention( hidden_features=self.hidden_representations, n_layers=3)

        self.dynamic_fusion = DynamicWeightedFusion(in_features=self.hidden_representations, n_inputs=4,share_weights=self.share_weighting).to(self.device)
        self.output_layer = torch.nn.Linear(self.hidden_representations, 1).to(self.device)


        self.kasa = KnowledgeAwareSelfAttention(self.hidden_representations)
    ...
    def forward(self, x, return_weights=False):
        ...
        #Knowledge-Aware Transformations
        outputs = self.kasa(outputs, graph_embed, self.dgl_graph)
        ka_labels = self.kasa(label_embed[None], graph_embed, self.dgl_graph)[0]

        # LSAN
        self_att = self.lsatt(outputs, label_embed)
        self_att2 = self.lsatt(outputs, ka_labels)


        label_att = self.latt(outputs, label_embed)

        graph_att = self.gsa(outputs, graph_embed, self.dgl_graph)
