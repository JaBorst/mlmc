...
class LSANOriginalTransformerNoClassesHierarchical(TextClassificationAbstractGraph):
    """
    https://raw.githubusercontent.com/EMNLP2019LSAN/LSAN/master/attention/model.py
    """
    def __init__(self, classes, method="glove", scale="mean", norm=False, representation="roberta", dropout=0.3, propagation_layers=3, d_a=200, max_len=400, n_layers=4, **kwargs):
        super(LSANOriginalTransformerNoClassesHierarchical, self).__init__(**kwargs)
        #My Stuff
        ...
        self.projection = torch.nn.Linear(self.embeddings_dim, self.label_embedding_dim * 2).to(self.device)
        ...
        self.weight3 = torch.nn.Linear(self.label_embedding_dim * 2, 1)
    ...
    def forward(self, x):
        ...
        graph1 = torch.bmm(label.expand(x.shape[0], *label.shape), h1.transpose(1, 2))
        graph2 = torch.bmm(label.expand(x.shape[0], *label.shape), h2.transpose(1, 2))
