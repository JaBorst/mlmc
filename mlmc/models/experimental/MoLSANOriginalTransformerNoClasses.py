...
class MoLSANOriginalTransformerNoClasses(TextClassificationAbstractMultiOutput):
    """
    https://raw.githubusercontent.com/EMNLP2019LSAN/LSAN/master/attention/model.py
    """
    def __init__(self, classes, method="glove", scale="nothing", norm=False, representation="roberta",  dropout=0.5, d_a=400, max_len=400, n_layers=4, **kwargs):
        super(MoLSANOriginalTransformerNoClasses, self).__init__(**kwargs)
        #My Stuff
        ...
        self.linear_second =torch.nn.ModuleList([torch.nn.Linear(self.label_embedding_dim, d_a) for _ in range(len(self.n_classes))])
    ...
    def forward(self, x):
        ...
        doc = [self.docs(outputs,i) for i in range(len(self.n_classes))]
    ...
    def label_spec_doc_rep(self, x,i):
        ...
        selfatt = torch.matmul(selfatt, self.linear_second[i](l).t())
    ...
    def adaptive_fusion(self,x,y,i):
    ...
    def docs(self, outputs, i):
        return self.adaptive_fusion(self.label_spec_doc_rep(outputs,i), self.label_spec_label_rep(outputs, i),i)
