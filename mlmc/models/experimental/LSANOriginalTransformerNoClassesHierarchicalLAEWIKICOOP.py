...
class LSANOriginalTransformerNoClassesHierarchicalLAEWikiCoop(TextClassificationAbstractGraph):
    """
    https://raw.githubusercontent.com/EMNLP2019LSAN/LSAN/master/attention/model.py
    """
    def __init__(self, classes, method="glove", scale="mean",  representation="roberta", dropout=0.3, propagation_layers=3, d_a=200, max_len=400, n_layers=4, **kwargs):
        super(LSANOriginalTransformerNoClassesHierarchicalLAEWikiCoop, self).__init__(**kwargs)
        #My Stuff
        ...
        self.linear_first = torch.nn.Linear(self.label_representation_size * 2, d_a).to(self.device)
        ...
        self.K=10
        self.label_ECR = ExpertCoRep(in_features=self.label_representation_size,
                                     out_features=self.label_representation_size,
                                     centers=torch.rand(self.K,self.label_representation_size),
                                     trainable=True)
    ...
    def forward(self, x, return_weights=False):
        ...
        h2 = outputs[:, :, self.label_representation_size:]
