...
class ZAGCNNLMMatrix(TextClassificationAbstract):
    def __init__(self, classes,   adjacency, representation="roberta", max_len=200, norm=False, dropout = 0.5, **kwargs):
        ...
        l = torch.cat(self.embedding(self.tokenizer(list(self.classes.keys()), 6))[2][(-1 - self.n_layers):-1], -1)
        if norm: l = l / l.norm(p=2, dim=-1, keepdim=True)
        l_cls = self.embedding(self.tokenizer(list(self.classes.keys()), 6))[1]
        if norm: l_cls = l_cls / l_cls.norm(p=2, dim=-1, keepdim=True)
        ...
        self.label_embeddings_cls = torch.nn.Parameter(l_cls)
        self.label_embeddings_cls.requires_grad = False
        self.label_embeddings_cls_dim = self.label_embeddings_cls.shape[-1]
        ...
        self.projection = torch.nn.Linear(in_features=self.embedding_dim, out_features=self.hidden_dim+self.label_embeddings_cls.shape[-1])
        self.build()
    ...
    def forward(self, x):
        ...
        a = torch.softmax(torch.einsum("ijk,lmk->ijlm",d2, self.label_embeddings).sum(-1), -1)
        ...
        labelgcn = self.gcn1(self.label_embeddings_cls, torch.stack(torch.where(self.adjacency_param == 1), dim=0))
        labelgcn = self.dropout_layer(labelgcn)
        labelgcn = self.gcn2(labelgcn, torch.stack(torch.where(self.adjacency_param == 1), dim=0))
        labelvectors = torch.cat([self.label_embeddings_cls, labelgcn], dim=-1)
