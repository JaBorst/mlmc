...
class KMemoryGraph(SentenceTextClassificationAbstract, TextClassificationAbstractZeroShot):
    ...
    def update_memory(self):
        ...
        self.memory = {
            k: [k] +self.map[k]+[x for x in sum([list(graph.neighbors(x)) for x in self.map[k]][:10] , [])] # if graph.nodes(True)[x]["pos"] in self._config["pos"]
            for k in self.classes.keys()
        }

        subgraph = {
            k: graph.subgraph(self.map[k] + [x for x in sum([list(graph.neighbors(x)) for x in self.map[k] ][:10], [])])
            for k in self.classes.keys()
        }
    ...
    def forward(self, x):
        ...
        keyword_similiarity_mean = torch.stack([self._sim(input_embedding, x).mean(-1) for i,(k, x) in enumerate(memory_embedding.items())],-1) # keyword-similarity-mean
