...
class MoKimCNNH(TextClassificationAbstractMultiOutput):
    """
    ...
    """
    def __init__(self, n_outputs, classes, hierarchy, mode="transformer", representation="roberta", kernel_sizes=[3,4,5,6], filters=100, dropout=0.5, n_layers=1, max_len=200, **kwargs):
        ...
        :param kwargs: Optimizer and loss function keyword arguments, see `mlmc.models.TextclassificationAbstract`
         """
        super(MoKimCNNH, self).__init__(n_outputs = n_outputs, **kwargs)
        ...
        self.mappings = [torch.zeros((self.n_classes[i], self.n_classes[i+1])) for i in range(len(self.n_classes)-1)]
        for x,y,z in hierarchy:
            self.mappings[0][self.classes[0][x], self.classes[1][y]]=1
            self.mappings[1][self.classes[1][y], self.classes[2][z]]=1
        self.mappings = torch.nn.ParameterList([torch.nn.Parameter(x) for x in self.mappings])
        self.mappings.requires_grad = False
    ...
    def forward(self, x):
        ...
        output[1] = (output[0].unsqueeze(-1)* ((self.mappings[0]).unsqueeze(0))).sum(-2) * output[1]
        output[2] = (output[1].unsqueeze(-1)* (self.mappings[1]).unsqueeze(0)).sum(-2) * output[2]
