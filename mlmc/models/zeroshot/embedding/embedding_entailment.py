...
class EmbeddingBasedEntailment(SentenceTextClassificationAbstract,TextClassificationAbstractZeroShot):
    ...
    def __init__(self, mode="vanilla",  *args, **kwargs):
        ...
        self.bottle_neck = 512
        self.parameter = torch.nn.Linear(self.embeddings_dim,self.bottle_neck)
        self.parameter2 = torch.nn.Linear(self.bottle_neck + self.embeddings_dim,self.embeddings_dim)
        self.att = torch.nn.TransformerEncoder(torch.nn.TransformerEncoderLayer(d_model=self.bottle_neck+self.embeddings_dim, nhead=8),num_layers=4)
        self.entailment_projection = torch.nn.Linear(4*self.embeddings_dim, self.bottle_neck)
    ...
    def forward(self, x, *args, **kwargs):
        ...
        if self._config["target"] == "entailment":
            ...
            interaction = torch.cat([input_embedding,interaction],-1)
            ...
            e = torch.cat([input_embedding, label_embedding, torch.abs(input_embedding - label_embedding),interaction],-1)
            # e = interaction
        else:
            le = self.parameter(label_embedding)
            ...
            interaction = torch.cat([input_embedding[:,None].repeat(1,interaction.shape[1],1,1),interaction],-1)
            ...
            sum_embeddings = torch.sum(interaction * input_mask_expanded, 2)/sum_mask
            input_embedding = self._mean_pooling(input_embedding, x["attention_mask"])
            label_embedding = self._mean_pooling(label_embedding, self.label_dict["attention_mask"])
