...
class EmbeddingBasedEntailment(SentenceTextClassificationAbstract,TextClassificationAbstractZeroShot):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        if self._config["target"]=="entailment":
            r = self.entailment_projection(
                torch.cat([input_embedding, label_embedding, torch.abs(input_embedding - label_embedding)],
                          -1)).squeeze(-1)
        else:
            r = self.entailment_projection(
                torch.cat([input_embedding[:,None].repeat(1,label_embedding.shape[0],1),
        ...
        if self._config["target"] == "entailment":
            ...
            return torch.log(r[..., -1].softmax(-1))
        elif self._config["target"] == "multi":
            return torch.log(r[..., [0, 2]].softmax(-1)[:, -1])
